...
from sqlalchemy import not_, func, case, and_
    ...
    def reports_act(self):
        ...
        ###################### ARSTS
            ...
            query = DBSession.query(ARSts.id, 
              ...
              ARInvoice.kode.label('no_bayar')
              ).filter(ARSts.unit_id==Unit.id, 
              ARSts.id==ARStsItem.sts_id, 
              # iWan Mampir
              ARSspd.id==ARStsItem.sspd_id, 
              ARInvoice.id==ARSspd.arinvoice_id, 
              # end
...
class r400Generator(JasperGenerator):
    ...
    def generate_xml(self, row):
        ...
        ET.SubElement(xml_greeting, "no_polisi").text = row['no_polisi'][0:2]

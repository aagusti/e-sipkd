...
SESS_ADD_FAILED = 'Gagal tambah wp SPTPD'
SESS_EDIT_FAILED = 'Gagal edit wp SPTPD'

########                    
# List #
########    
@view_config(route_name='sptpd-wajib', renderer='templates/wp_sptpd/list.pt',
             permission='read')
...
@view_config(route_name='sptpd-wajib-add', renderer='templates/wp_sptpd/add.pt',
             permission='add')
            ...
            if e == 'True':
                if b != '':
                    ...
                    if cek :
                        ...
                        return HTTPFound(location=request.route_url('sptpd-wajib-add'))
                else:
...
def id_not_found(request):    
    msg = 'Wajib Pajak ID %s not found.' % request.matchdict['id']
    request.session.flash(msg, 'error')
    return route_list(request)

@view_config(route_name='sptpd-wajib-edit', renderer='templates/wp_sptpd/edit.pt',
             permission='edit')
        ...
        if 'simpan' in request.POST:
            ...
            if e == 'True':
                if b != '':
                    ...
                    if cek :
                        request.session.flash('Email sudah digunakan.', 'error')
                        return HTTPFound(location=request.route_url('sptpd-wajib-edit',id=row.id))
                else:
                    request.session.flash('Email harus diisi.','error')
                    return HTTPFound(location=request.route_url('sptpd-wajib-edit',id=row.id))
...
# Delete #
##########    
@view_config(route_name='sptpd-wajib-delete', renderer='templates/wp_sptpd/delete.pt',
             permission='delete')
...
# Action #
##########    
@view_config(route_name='sptpd-wajib-act', renderer='json',
             permission='read')
def view_act(request):
    ...
    if url_dict['act']=='grid':
        ...
        query = DBSession.query(SubjekPajak).join(Unit).filter(SubjekPajak.status_grid==1)
...
# CSV #
########          
@view_config(route_name='sptpd-wajib-csv', renderer='csv')
...
def view_pdf(request):
    ...
    if url_dict['pdf']=='reg' :
        ...
        rml_row = open_rml_row('wp_sptpd.row.rml')
        rows=[]
        for r in query.all():

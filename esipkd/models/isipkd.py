...
class UnitRekening(Base):
    ...
    unit_id     = Column(Integer, ForeignKey('units.id'),     primary_key=True)
    rekenings   = relationship("Rekening", backref=backref('unit_rekenings'))  
    units       = relationship("Unit",     backref=backref('unit_rekenings'))
    
    @classmethod
    def _get_by_rek(cls, rekening):
        return DBSession.query(cls).filter_by(rekening_id=rekening.id).all()
        
    @classmethod
    def get_by_rek(cls, rekening):
        units = []
        for g in cls._get_by_rek(rekening):
            units.append(g.unit_id)
        return units
        
    @classmethod
    def set_one(cls, session, rekening, unit):
        member = DBSession.query(cls).filter_by(rekening_id=rekening.id, unit_id=unit.id)
        try:
            member = member.one()
        except NoResultFound:
            member = cls(rekening_id=rekening.id, unit_id=unit.id)
            DBSession.add(member)
            transaction.commit()
...
class Sektor(NamaModel, Base):
    __tablename__ = 'sektors'
    status        = Column(Integer, default=0)
    UniqueConstraint('kode')         
    
    @classmethod
    def get_by_nama(cls, nama):
        return DBSession.query(cls).filter(cls.nama==nama).all()
    
class Produk(NamaModel, Base):
...
class Peruntukan(NamaModel, Base):
    __tablename__ = 'peruntukans'
    status        = Column(Integer, default=0)
    UniqueConstraint('kode')              
...
class JnsPajak(NamaModel, Base):
    __tablename__ = 'jns_pajaks'
    status        = Column(Integer, default=0)
    UniqueConstraint('kode')        
...
class ARStsItem(Base):
    ...
    invoice_id    = Column(Integer, ForeignKey('arinvoices.id'), primary_key=True)
    rekening_id   = Column(Integer, ForeignKey('rekenings.id'),  primary_key=True)
    jumlah        = Column(BigInteger, nullable=False, default=0)
    rek_kode      = Column(String(16))    
    rek_nama      = Column(String(64))
    ...
    arinvoices    = relationship("ARInvoice", backref=backref('arstsitems'))
...
class ARTbp(CommonModel, Base):
    ...
    invoice_id      = Column(Integer,      nullable=True,  default=0) # Penambahan baru (29122016)
    invoice_kode    = Column(String(32)) # Penambahan baru (29122016)
    create_uid      = Column(Integer)
    update_uid      = Column(Integer)
    create_date     = Column(DateTime(timezone=True))
    update_date     = Column(DateTime(timezone=True))

    subjekpajaks    = relationship("SubjekPajak", backref=backref('artbp'))
